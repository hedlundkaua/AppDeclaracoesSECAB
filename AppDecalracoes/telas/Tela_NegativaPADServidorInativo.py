...
def abrir_tela():
    ...
    def gerar_documento():
        try:
            ...
            dataAposentadoria = entry_aposenta.get()
            genero = genero_var.get()
            servidor_assinador = combo_servidor.get()
    

            #caminho do modelo
            modelo_path = caminho_arquivo("W:\DRH\SECAB\Kaua Teste\modelos\declaracao_DeNegativaEmProcessoAdministrativoPADServidorInativo.docx")
            ...
            contexto = {
                ...
                "classe": classe,
                "dataNomeacao": dataDaPublicacao,
                "dataDeExercicio": dataInicioExe,
                "cargoDeQuandoAposentado": cargoAtual,
                "dataDaAposentadoria": dataAposentadoria,
                "dataDoDoedeAposentadoria": dataAposentadoria,
                "data": data_arquivo,
                "genero": genero
            }
    ...
    # cargo atual 3
    tk.Label(janela, text="Cargo Quando o Servidor se Aposentou:").grid(row=3, column=0, sticky="e")
    ...
    tk.Label(janela, text="Data da Nomeação:").grid(row=5, column=0, sticky="e")
    ...
    # inicio exercicio 6
    tk.Label(janela, text="Inicio do Inicio de Exercicio:").grid(row=6, column=0, sticky="e")
    ...
    # data da aposentadoria # 7
    tk.Label(janela, text="Data da Aposentadoria:").grid(row=7, column=0, sticky="e")
    entry_aposenta = tk.Entry(janela, width=40)
    entry_aposenta.grid(row=7, column=1)
    entry_aposenta.bind("<KeyRelease>", lambda event: fd.formatar_data(event, entry_aposenta)) 
    ...
    ttk.Label(janela, text="Quem vai assinar:").grid(row=11, column=0, sticky="e")
