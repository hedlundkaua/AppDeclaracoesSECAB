...
def abrir_tela():
    def gerar_documento():
        try:
            ...
            cargoDaNomeacao = entry_classe.get()
            ...
            #caminho do modelo
            modelo_path = caminho_arquivo("W:\DRH\SECAB\Kaua Teste\modelos\declaracao_InscricaoOABeDeclaracaoDePoderDdecisorio.docx")
            ...
            contexto = {
                ...
                "classe": classe,
                "cargoDaNomeacao": cargoDaNomeacao,
            ...
            nome_arquivo = f"{nome.replace(' ','_')}_declaracao_InscricaoOABeDeclaracaoDePoderDdecisorio {data_hoje}.docx"
            ...
            if not nome or not id or not cpf or not rg or not cargoDaNomeacao or not cargoAtual or not classe or not setorAtual or not rg or not dataDaPublicacao or not dataInicioExe:
    ...
    janela = tk.Tk()
    janela.title("Declaração de inscrição OAB")
    ...
    tk.Label(janela, text="Cargo de nomeação:").grid(row=6, column=0, sticky="e")
    entry_cargo = tk.Entry(janela, width=40)
    entry_cargo.grid(row=6, column=1)
    ...
    # Data da publicação
    tk.Label(janela, text="Data da publicação:").grid(row=8, column=0, sticky="e")
    entry_publicacao = tk.Entry(janela, width=40) #onde o usuario vai digitar
    entry_publicacao.grid(row=8, column=1) 
    ...
    tk.Label(janela, text="Inicio do Exercicio:").grid(row=9, column=0, sticky="e")
    entry_exercicio = tk.Entry(janela, width=40)
    entry_exercicio.grid(row=9, column=1)
    ...
    tk.Label(janela, text="Genero:").grid(row=10, column=0, sticky="e")
    tk.Radiobutton(janela, text="Masculino", variable=genero_var, value="Masculino").grid(row=10, column=1, sticky="w")
    tk.Radiobutton(janela, text="Feminino", variable=genero_var, value="Feminino").grid(row=11, column=1, sticky="w")
    ...
    tk.Label(janela, text="Quem vai assinar:").grid(row=12, column=0, sticky="e")
    combo_servidor = ttk.Combobox(janela, values=list(servidores.keys()), width= 37)
    combo_servidor.grid(row=12, column=1)
