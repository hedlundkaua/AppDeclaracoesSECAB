...
def abrir_tela():
    def gerar_documento():
        try:
            ...
            #caminho do modelo
            modelo_path = caminho_arquivo("W:\DRH\SECAB\Kaua Teste\modelos\declaracao_DasCompetenciasDosATREs.docx")
            ...
            nome_arquivo = f"{nome.replace(' ','_')}_declaracao_DasCompetenciasDosATRE {data_hoje}.docx"
            ...
            if not nome or not id or not cpf or not pagina or not rg or not dataDaPublicacao or not dataInicioExe:
    ...
    janela = tk.Tk()
    janela.title("Declaração das competencas dos ATREs")
    ...
    tk.Label(janela, text="Setor Atual:").grid(row=8, column=0, sticky="e")
    entry_setor = tk.Entry(janela, width=40)
    entry_setor.grid(row=8, column=1)
